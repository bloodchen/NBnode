...
const nidLoader = new NbCore.NidLoader;
...
app.get('/', async function (req, res, next) {
    ...
    try {
        ...
        if (domain.indexOf("@") !== -1) {
            const ret = await nidLoader.readUserProperty(domain,f);
            res.json(ret);
        } else {
            const ret = await nidLoader.readLocalPredictNid(domain,f);
            res.json(ret);
        }
...
app.get(`/find_domain`, function (req, res) {
    try {
        ...
        var addr = q.address;
        let f = (q.full == 'true');
        let result = nidLoader.findDomain('owner', addr, !f);
        res.json({
            code: 0,
            message: "OK",
            obj: result
