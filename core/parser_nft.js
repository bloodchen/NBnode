...
const { Parser_Domain } = require("./parser_domain.js");
...
class CMD_NFT_Create {
    static parseTX(rtx) {
        ...
        const nft = Parser_NFT.db.getNFT(output.symbol)
        if (nft && !testing) {
    ...
    static fillObj(nidObj, rtx) {
        ...
        const nft = Parser_NFT.db.getNFT(symbol)
        if (nft && !testing){
        ...
        Parser_NFT.db.nftCreate(rtx.output) //create or update the NFT
...
class CMD_NFT_Transfer {
    ...
    static _verifyTransfer(output,nidObj=null,objMap=null){
        ...
        if(nidObj==null) nidObj = Parser_NFT.db.loadDomain(output.domain)
        ...
        for (const item of transInfo) {
            ...
            if(desObj==null)
                desObj = Parser_NFT.db.loadDomain(item.to)
            if(!desObj){
